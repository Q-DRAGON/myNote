...
// 转义符是字符串中的特殊符号，由反斜杠（blackslash）开始，接另一个字符结束
// 常用的转义符有
...
const process = function(array, processor) {
    /*
    array 是一个数组
    ...
    */
    ...
    for (let i = 0; i < array.length; i++) {
        let a = array[i]
        // processor 必须能调用成功，否则这里就报错了
        let element = processor(a)
        l.push(element)
    }
    return l
}
...
// String 内置函数
let stringList = process(array, String)
...
// 用上面的 procee 函数处理试试
let array = [1, 2, 3]
let squareList = process(array, square)
log('square list', squareList)

const addList = process(array, function(n) {
    // 我们定义一个接受一个参数的函数，并且直接使用，它左右名字
    return n + 1
})
