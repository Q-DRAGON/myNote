...
//    8	path.extname(p)
//    返回路径中文件的后缀名，即路径中最后一个'.'之后的部分。如果一个路径中并不包含'.'
...
// 转换为绝对路径
console.log('resolve : ' + path.resolve('工具模块.js'));

// 路径中文件的后缀名
console.log('ext name : ' + path.extname('工具模块.js'));
...
// 监听指定端口 port 和 主机 host ac连接。 默认情况下 host 接受任何 IPv4
// 地址(INADDR_ANY)的直接连接。端口 port 为 0 时，则会分配一个随机端口。
...
// 8	server.ref()
// 与 unref 相反，如果这是唯一的服务器，在之前被 unref 了的服务器上调用 ref
...
// net.Socket
//
// net.Socket 对象是 TCP 或 UNIX Socket 的抽象。net.Socket
// 实例实现了一个双工流接口。 他们可以在用户创建客户端(使用 connect())时使用,
// 或者由 Node 创建它们，并通过 connection 服务器事件传递给用户。
// 事件
//
// net.Socket 事件有：
// 序号	事件 & 描述
...
// 14	socket.unref()
// 如果这是事件系统中唯一一个活动的服务器，调用 unref 将允许程序退出。如果服务器已被 unref，
// 则再次调用 unref 并不会产生影响。
// 15	socket.ref()
